...
unsigned int nStakeMinAge = 1 * 60; // 1 day
...
const string strMessageMagic = "Chingeling Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // Chingeling: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // Chingeling: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// Chingeling: find last block index up to pindex
...
unsigned int static GetNextWorkRequired_legacy(const CBlockIndex* pindexLast)
{
    ...
    // Chingeling: This fixes an issue where a 51% attack can change difficulty at will.
...
static unsigned int GetNextTargetRequired_(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Includes Chingeling fix for wrong retargeting difficulty by Mammix2
...
unsigned int static DarkGravityWave3POS(const CBlockIndex* pindexLast, bool fProofOfStake) {
    /* current difficulty formula, darkcoin - DarkGravity v3, written by Evan Duffield - evan@darkcoin.io, DarkGravity v3 - POS modified by "anush3070"*/
    CBigNum bnTargetLimit = fProofOfStake ? bnProofOfStakeLimit : bnProofOfWorkLimit;
...
{
    return DarkGravityWave3POS(pindexLast,fProofOfStake);
}
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    // fMiner is true when called from the internal Chingeling miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Chingeling: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // Chingeling: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if(IsProofOfWork())
    {
        CChingelingAddress address(!fTestNet ? FOUNDATION : FOUNDATION_TEST);
    ...
    if (IsProofOfStake())
    {
        // Chingeling: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // Chingeling: track money supply and mint amount info
...
// Chingeling: total coin age spent in transaction, in the unit of coin-days.
...
// Chingeling: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // Chingeling: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // Chingeling: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // Chingeling: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // Chingeling: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    // Chingeling: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // Chingeling: check proof-of-stake
    ...
    // Chingeling: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // Chingeling: getblocks may not obtain the ancestor block rejected
    ...
    // Chingeling: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Chingeling", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Chingeling development";
        CTransaction txNew;
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("88c695698f9bd26b816d00b3387e44a63119bc19620be667ab45814cf591fb1f"));
        block.print();
        ...
        // Chingeling: initialize synchronized checkpoint
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // Chingeling: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // Chingeling: tell downloading node about the latest block if it's
